...
// --- messages
enum {
	GUI_DID_OPEN,				/* RAFX GUI called after GUI_RAFX_OPEN, NOT called with GUI_USER_CUSTOM_OPEN */
...
// --- Messages
enum { preProcessData, postProcessData, updateHostInfo, idleProcessData };
enum { vectorJoystickX_Offset, vectorJoystickY_Offset, numAddtlParams }; // numAddtlParams ALWAYS LAST

const UINT MAX_USER_METERS = 20;
...
#endif
	GUI_PARAMETER* pGUIParameters;	// ptr to GUI_PARAMETERs
...
{
public:
	CFloatParamSmoother() { a = 0.0; b = 0.0; z = 0.0; }
	...
	inline float smoothParameter(float in)
	{
		z = (in * b) + (z * a);
		return z;
	}
...
const UINT CURRENT_PLUGIN_API = 63;
...
const UINT RESERVED_5 = 5;	// future use
const UINT RESERVED_6 = 6;	// future use
const UINT RESERVED_7 = 7;	// future use
// -------------------------------------------------------------------------
